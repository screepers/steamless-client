export function addScreepsAudio() {
    ...
    // https://freesound.org/people/JustASeriesofFonetics/sounds/838158/
    const rangedAttackAudio = [
        new Audio('https://cdn.freesound.org/previews/838/838158_12376155-lq.ogg'),
        new Audio('https://cdn.freesound.org/previews/838/838158_12376155-lq.ogg'),
        new Audio('https://cdn.freesound.org/previews/838/838158_12376155-lq.ogg'),
        new Audio('https://cdn.freesound.org/previews/838/838158_12376155-lq.ogg'),
        new Audio('https://cdn.freesound.org/previews/838/838158_12376155-lq.ogg'),
    ...
    window.WebSocket = function (a, b) {
        ...
        that.addEventListener('message', (m) => {
            ...
            const soundController = window.document.getElementById('audio-checkbox');
            if (!soundController) {
            ...
            const rangedAttackCount = (tickDataString.match(/"rangedAttack"/g) || []).length;
            if (rangedAttackCount) {
                console.log(`rangedAttackCount: ${rangedAttackCount}`);
                for (let i = 0; i < Math.min(healAudio.length, healedCount); i++) {
                    setTimeout(() => {
                        rangedAttackAudio[i].load();
                        rangedAttackAudio[i].play();
                    }, Math.random() * 300);
                }
            }
            //
             ...
             */
            let currentTickDataObj: { [x: string]: any } = {};
            try {
            ...
            let dmgIndex = 0; // so there are more dmg sounds overlapping
            const changedObjects = Object.keys(currentTickDataObj);
            for (const id of changedObjects) {
                if (currentTickDataObj[id] == null) {
                    continue;
                }
                ...
                if (currentTickDataObj[id].hits && pTickData[id]?.hits) {
                    ...
                    if (currentTickDataObj[id].hits < pTickData[id].hits) {
                        ...
                        setTimeout(() => {
                            damagedAudio[Math.min(dmgIndex, damagedAudio.length)].load();
                            damagedAudio[Math.min(dmgIndex, damagedAudio.length)].play();
                        }, Math.random() * 300);
                        dmgIndex++;
                    }
                }
